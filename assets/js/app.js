(() => {
  const TEMPLATE_LIBRARY = [
    ...
    { id: "personal_creator", name: "Creator Launch", category: "Personal", headline: "Grow Your Audience With A Simpler Creator Message", subheadline: "Cut noise, tighten the promise, and make subscribing feel like a smart move today.", cta: "Join The List", alignment: "center", layoutType: "minimal", spacing: 55, background: "bg-3", tone: "minimal" }
  ];
  ...
  function createDefaultEditorState() {
    return {
      alignment: "left",
      headline: { topOffset: 0, fontIndex: 1 },
      subheadline: { topOffset: 0, fontIndex: 1 },
      cta: { topOffset: 0, fontIndex: 1, width: "medium" }
    };
  }
  ...
  function cloneSlide(slide) {
    return JSON.parse(JSON.stringify(window.SlideEngine.normalizeSlideLayout(slide)));
  }

  function setBaselineSlide(slide) {
    state.originalSlide = cloneSlide(slide);
    state.originalDesign = { ...state.slideDesign };
    state.editor = createEditorStateFromSlide(slide);
  ...
  function applyOptimization(options = {}) {
    const settings = {
      ...
      label: options.label || "Improved by Smart Engine",
  ...
  function moveField(field, direction) {
    ...
    state.editor[field].topOffset += step;
    commitEditorUpdate();
  }
  ...
  function resetSlide() {
    ...
    state.editor.alignment = state.currentSlide.alignment || "left";
    state.activeAlignment = state.editor.alignment;
    state.autopilotInsight = null;
    state.selectedField = null;
    renderCurrentSlide();
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .hero-slide__headline{margin:0;max-width:min(${headlineMaxWidth}px,100%);font-size:clamp(2.5rem,${snapshot.fontSize.headline}vw,${snapshot.fontSize.headline}rem);line-height:${headlineLineHeight};font-weight:800;letter-spacing:-.05em;opacity:0;animation:headlineFade .52s ease forwards}
    .hero-slide__subheadline{margin:0;max-width:min(${subheadlineMaxWidth}px,100%);font-size:clamp(1rem,${Math.max(1.6, snapshot.fontSize.subheadline * 1.45)}vw,${snapshot.fontSize.subheadline}rem);line-height:${subheadlineLineHeight};color:rgba(245,247,251,.78);opacity:0;animation:bodyFade .46s ease .15s forwards}
    .hero-slide__cta{display:inline-flex;align-items:center;justify-content:center;min-height:58px;margin-top:${ctaMarginTop}px;padding:0 24px;max-width:min(${ctaMaxWidth}px,100%);border-radius:999px;backdrop-filter:blur(14px);font-size:clamp(.95rem,${Math.max(1.4, snapshot.fontSize.cta * 1.1)}vw,${snapshot.fontSize.cta}rem);font-weight:700;text-decoration:none;opacity:0;animation:ctaFade .42s cubic-bezier(.2,.9,.2,1) .3s forwards;${ctaStyles}}
    ...
    @keyframes headlineFade{from{opacity:0;transform:translateY(24px)}to{opacity:1;transform:translateY(0)}}
    @keyframes bodyFade{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}
    @keyframes ctaFade{from{opacity:0;transform:scale(.92)}to{opacity:1;transform:scale(1)}}
...
<body>
  ...
  function exportHtml() {
    ...
    const url = downloadFile(markup, `${slide.id || "smart-slide"}.html`, "text/html");
  ...
  function handleInput(event) {
    ...
    if (event.target === dom.headlineInput) {
      updateSlideText("headline", event.target.value);
      return;
    }

    if (event.target === dom.subheadlineInput) {
      updateSlideText("subheadline", event.target.value);
      return;
    }

    if (event.target === dom.ctaInput) {
      updateSlideText("cta", event.target.value);
      return;
    }
